...
<div
    class="fixed top-0 left-0 w-full bg-{{ $template_config->template_color }}-100 border-b-4 border-{{ $template_config->template_color }}-500 text-{{ $template_config->template_color }}-700 p-4 z-50">
    <div class="flex flex-col sm:flex-row sm:items-center items-start justify-between max-w-screen-lg mx-auto">
        <!-- Badge -->
        <div class="flex items-center mb-3 sm:mb-0">
            <span
                class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-{{ $template_config->template_color }}-500 text-white">
        ...
        @if ($secret != '')
            <span class="flex items-center lg:ml-2 text-sm text-left sm:text-center">
                ...
                <a href="{{ route('admin.plugin.maintenance-mode') }}"
                    class="text-{{ $template_config->template_color }}-600 hover:text-{{ $template_config->template_color }}-700 ml-2">
        ...
        <div class="relative w-full max-w-5xl p-8 bg-white rounded-lg shadow-lg z-10">
            ...
            <div class="text-center text-gray-500">
                ...
                <div class="flex items-center justify-center mt-4">
                    <h2 id="secret-code" class="text-gray-900 font-semibold text-4xl break-words">{{ $secret }}</h2>
                    <button onclick="copySecretCode()"
                        class="ml-3 p-2 text-white bg-{{ $template_config->template_color }}-600 rounded hover:bg-{{ $template_config->template_color }}-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-{{ $template_config->template_color }}-500">
                ...
                <p class="text-sm text-gray-500 mt-4">
                    {{ __('For example:') }} <a href="{{ config('app.url') }}/admin/plugin/maintenance-mode" target="_blank"
