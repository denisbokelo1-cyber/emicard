@php
    ...
    $setting = GoBizCommonService::settings();
    $pages = GoBizCommonService::templatePageContentAll('GoBizOriginal');
    $homePage = GoBizCommonService::templatePageContentGet('home', 'GoBizOriginal');
    ...
    $modernDashboardEnabled =
        Schema::hasTable('modern_dashboard_settings') && is_dir(base_path('plugins/ModernDashboard'))
            ? (int) DB::table('modern_dashboard_settings')->value('modern_dashboard_enabled')
            : 0;
    ...
    $dashboardUrl =
        $user && $user->role_id != 2
            ...
            : route(
                $modernDashboardEnabled && $user && $user->role_id == 2 ? 'user.dashboard.overview' : 'user.dashboard',
            );
@endphp
...
<section
    class="bg-{{ request()->is('/') ? 'gradient-' . $template_config->template_color . '2' : 'white' }} lg:px-20 background-animate">
    <section>
                                ...
                                <div class="w-auto hidden xl:block">
                                    <ul class="flex items-center mr-10">
                                        @if ($pages[204]->page_name == 'about' && $pages[204]->status == 'active')
                                            <li
                                                class="font-heading mr-9 text-gray-900 hover:text-gray-700 {{ App::isLocale('ru') ? 'text-md' : 'text-lg' }}">
                                                <a href="{{ route('about') }}">{{ __('About') }}</a>
                                            </li>
                                        @endif
                                        <li
                                            class="font-heading mr-9 text-gray-900 hover:text-gray-700 {{ App::isLocale('ru') ? 'text-md' : 'text-lg' }} truncate">
                                            <a
                                                href="{{ route('home-locale') }}#how-it-works">{{ __('How it works?') }}</a>
                                        </li>
                                        <li
                                            class="font-heading mr-9 text-gray-900 hover:text-gray-700 {{ App::isLocale('ru') ? 'text-md' : 'text-lg' }}">
                                            <a href="{{ route('home-locale') }}#features">{{ __('Features') }}</a>
                                        </li>
                                        <li
                                            class="font-heading mr-9 text-gray-900 hover:text-gray-700 {{ App::isLocale('ru') ? 'text-md' : 'text-lg' }}">
                                            <a href="{{ route('home-locale') }}#pricing">{{ __('Pricing') }}</a>
                                        </li>
                                        ...
                                        @if ($config[88]->config_value == '1')
                                            <button
                                                class="font-heading mr-9 text-gray-900 {{ App::isLocale('ru') ? 'text-md' : 'text-lg' }} text-center inline-flex items-center truncate"
                                                type="button"
                    ...
                    <div class="w-auto hidden xl:block">
                        <div class="flex flex-nowrap">
                            ...
                            @if (!$isDown)
                                ...
                                <div class="w-auto">
                                    @if (count(config('app.languages')) > 1)
                                        <div class="relative">
                                            <button
                                                class="font-heading mr-4 mt-2 text-gray-900 {{ App::isLocale('ru') ? 'text-md' : 'text-lg' }} text-center inline-flex items-center"
                            ...
                            @if ($config[38]->config_value != 'no')
                                @guest
                                    <div class="w-auto">
                                        <a class="font-heading block py-2 px-5 mr-5 {{ App::isLocale('ru') ? 'text-md' : 'text-lg' }} text-gray-900 rounded-xl truncate"
                                            href="{{ route('login') }}">{{ __('Sign In') }}</a>
                                    </div>

                                    @if (Route::has('register'))
                                        <div class="w-auto">
                                            <a class="group relative font-heading block py-2 px-5 {{ App::isLocale('ru') ? 'text-md' : 'text-lg' }} text-gray-900 border-2 border-gray-900 overflow-hidden rounded-xl truncate"
                                                href="{{ route('register') }}">
                                                <div
                                    ...
                                    <div @click.away="open = false"
                                        ...
                                        <div x-show="open" x-transition:enter="transition ease-out duration-100"
                                            ...
                                            <div class="px-2 py-2 bg-white capitalize rounded-sm shadow">
                                                @foreach (config('app.languages') as $langLocale => $langName)
                                                    <a class="block px-4 py-2 mt-2 {{ App::isLocale('ru') ? 'text-md' : 'text-lg' }} capitalize font-semi-bold bg-transparent rounded-sm dark-mode:bg-transparent md:mt-0 focus:outline-none focus:shadow-outline"
                ...
                <div class="flex flex-wrap justify-between h-full">
                    ...
                    <div class="flex flex-col justify-center py-8 w-full">
                        <ul>
                            @if ($pages[204]->page_name == 'about' && $pages[204]->status == 'active')
                                <li class="mb-6"><a
                                        class="font-heading font-medium {{ App::isLocale('ru') ? 'text-md' : 'text-lg' }} text-gray-900 hover:text-gray-700"
                                ...
                                @foreach ($pages as $page)
                                    @if ($page->page_name == 'Custom Page' && $page->status == 'active')
                                        <li
                                            ...
                                            <a class="font-heading font-medium {{ App::isLocale('ru') ? 'text-md' : 'text-lg' }} text-gray-900 hover:text-gray-700"
                    ...
                    <div class="flex flex-col justify-end w-full">
                        <div class="flex flex-wrap">
                            ...
                            @guest
                                <div class="w-full">
                                    <a class="p-0.5 font-heading block w-full {{ App::isLocale('ru') ? 'text-md' : 'text-lg' }} text-gray-900 font-medium rounded-xl"
                                        href="{{ route('login') }}">
                                    ...
                                    <div class="w-full">
                                        <a href="{{ route('register') }}"
                                            ...
                                            <div class="py-2 px-5 bg-white rounded-lg">
                                                <p class="relative z-10 {{ App::isLocale('ru') ? 'text-sm' : '' }}">
                                                    {{ __('Try now') }}</p>
                                            </div>
                                        </a>
                                    </div>
            ...
            <div class="flex flex-wrap items-center -m-4 lg:pt-24 pt-10 pb-36">
                <div class="w-full lg:w-1/2 p-6">
                    <div class="lg:max-w-xl">
                        ...
                        <button
                            class="group relative font-heading px-10 py-5 my-2 w-full lg:w-auto uppercase text-white text-xs font-semibold tracking-px bg-{{ $template_config->template_color }}-800 overflow-hidden rounded-md">
                ...
                <div class="w-full lg:w-1/2 p-6 hidden md:block mobile-banner">
                    <img class="block mx-auto w-50" src="{{ asset($template_config->banner_image) }}"
