...
<section class="relative py-12 lg:px-24 overflow-hidden">
    <img class="absolute bottom-0 left-0" src="{{ asset('gobiz_original_assets/images/footers/radial.svg') }}"
        alt="">
    <div class="relative z-10 container mx-auto px-4">
        <div class="flex flex-wrap -m-6">
            <div
                class="w-full md:w-1/2 {{ $pages->where('page_name', 'Custom Page')->where('status', 'active')->isNotEmpty() ? 'lg:w-3/12' : 'lg:w-3/12' }} p-6">
            ...
            <div
                ...
                <div class="h-full">
                    ...
                    <ul>
                        {{-- NFC Card Ordering --}}
                        @if ($config[76]->config_value == '1' && $config[97]->config_value == '1')
                            <li class="mb-4"><a
                                    class="font-heading font-medium text-base text-gray-900 hover:text-gray-700"
                                    href="{{ route('nfc.cards') }}">{{ __('NFC Card Ordering') }}</a></li>
                        @endif
                        {{-- Directory Listing --}}
                        @if (is_dir(base_path('plugins/Directory')))
                            ...
                            @if ($directory_settings->directory == 1)
                                <li class="mb-4"><a
                                        class="font-heading font-medium text-base text-gray-900 hover:text-gray-700"
                                        href="{{ route('directory') }}">{{ __('Directory Listing') }}</a></li>
                            @endif
                        @endif
                        {{-- Custom Page --}}
            ...
            <div class="w-full p-6">
                <div class="h-full">
                    <p class="text-gray-600 text-sm">© {{ __('Copyright') }} {{ Carbon\Carbon::now()->format('Y') }}.
                        {{ __('All Rights Reserved by') }} {{ config('app.name') }}.</p>
                </div>
            </div>
        </div>
    </div>
