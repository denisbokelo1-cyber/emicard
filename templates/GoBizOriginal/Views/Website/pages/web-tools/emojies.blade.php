...
<script src="{{ asset('gobiz_original_assets/js/clipboard.min.js') }}"></script>
...
<div>
    <section class="text-gray-700">
        <div class="container px-5 py-12 mx-auto">
            ...
            <button class="emojiPicker group relative font-heading px-10 py-5 mb-8 w-full lg:w-auto uppercase text-white text-xs font-semibold tracking-px bg-gradient-to-r from-{{ $template_config->template_color }}-400 to-{{ $template_config->template_color }}-500 overflow-hidden rounded-md">{{ __('Start Picker') }}</button>

            <button class="copyBtn group relative font-heading px-10 py-5 w-full lg:w-auto uppercase text-white text-xs font-semibold tracking-px bg-gradient-to-r from-{{ $template_config->template_color }}-400 to-{{ $template_config->template_color }}-500 overflow-hidden rounded-md" data-clipboard-action="copy" data-clipboard-target="#content">{{ __('Copy to clipboard') }}</button>
        </div>
    </section>
</div>
...
{{-- Custom JS --}}
@section('custom-js')
<script src="{{ asset('gobiz_original_assets/js/EmojiPicker.js') }}"></script>
<script>
