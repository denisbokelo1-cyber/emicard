...
@section('custom-script')
    {{-- Custom Style --}}
    <link rel="stylesheet" href="{{ asset('gobiz_original_assets/css/custom-style.css') }}">
    ...
    <section class="py-5 lg:py-12 overflow-hidden">
        <div class="relative container px-4 mx-auto">
            <div class="text-center mb-12">
                <span
                    class="inline-block py-1 px-3 mb-4 text-xs font-semibold text-indigo-800 bg-{{ $template_config->template_color }}-50 rounded-full">{{ $blogDetails->blogCategory->blog_category_title }}</span>
                <h1 class="font-heading text-3xl xs:text-4xl md:text-5xl font-bold text-gray-800 mb-12">
                    <span>{{ __($blogDetails->heading) }}</span>
                </h1>
                <div>
                    <img class="block w-full lg:w-1/2 lg:h-1/2 lg:object-center mx-auto rounded-lg"
                        src="{{ asset($blogDetails->cover_image) }}" alt="{{ __($blogDetails->heading) }}">
                </div>
            </div>
                        ...
                        <div class="max-w-3xl mx-auto">
                            <div class="prose prose-lg prose-slate">
                                ...
                                @php
                                    ...
                                    $content = html_entity_decode($blogDetails->long_description);
                                @endphp
                            ...
                            <div class="flex flex-wrap -m-2">
                                ...
                                @foreach ($tags as $tag)
                                    <div class="w-auto p-2">
                                        <span
                                            class="relative group inline-block cursor-pointer w-full sm:w-auto py-1 px-3 mb-4 text-center text-sm font-semibold text-gray-50 hover:text-gray-50 bg-{{ $template_config->template_color }}-800 hover:bg-{{ $template_config->template_color }}-700 rounded-md overflow-hidden transform group-hover:translate-x-full group-hover:scale-102 transition duration-500">{{ strtoupper(__($tag)) }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                            ...
                            <div class="flex flex-wrap -m-2">
                                <div class="w-auto p-2">
                                    ...
                                    <a href="{{ route('sharetofacebook', $blogDetails->slug) }}" target="_blank"
                                        class="flex items-center justify-center p-3 text-white hover:text-gray-50 font-medium tracking-tighter bg-{{ $template_config->template_color }}-800 hover:bg-{{ $template_config->template_color }}-700 border-2 border-white focus:border-green-400 focus:border-opacity-40 focus:ring-4 focus:ring-green-400 focus:ring-opacity-40 rounded-full transition duration-300">
                            ...
                            <div class="pt-16">
                                ...
                                <div class="flex flex-wrap -mx-4 -mb-12">
                                    @foreach ($recentBlogs as $blog)
                                        <div
                                            class="w-full md:w-1/2 xl:w-1/3 p-4 mb-12 border-r border-gray-100 shadow shadow-lg rounded-lg">
                                            <a class="block group" href="{{ route('view.blog', $blog->slug) }}">
                                                ...
                                                <h4
                                                    ...
                                                    {{ __($blog->heading) }}</h4>
                                                <p class="text-gray-500">{{ __($blog->short_description) }}</p>
                                            </a>
                                        </div>
                                    @endforeach
                                </div>
